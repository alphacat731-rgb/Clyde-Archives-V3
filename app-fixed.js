(() => {
  ...
  function renderPagination(total) {
    ...
    if (pages <= 1) { nav.classList.add('hidden'); return; }
    ...
    const button = (label, disabled, handler, icon='') => {
      ...
      b.innerHTML=icon?`<span class="icon w-4 h-4">${ICONS[icon]}</span>`:label;
      b.onclick=handler; nav.appendChild(b);
    };
  ...
  function renderPage(){
    ...
    paintIcons();
    renderPagination(activeResults.length);
  }
  ...
  function runSearch(term){
    activeTerm=term.trim();
    activeResults=DATA.filter(item=>searchMatches(item,activeTerm));
    currentPage=1;
    renderPage();
    updateStatus(activeTerm);
    if(!activeResults.length){
      ...
      suggestions.querySelectorAll('[data-suggest]').forEach(b=>b.onclick=()=>{input.value=b.dataset.suggest;runSearch(b.dataset.suggest)});
    }else suggestions.innerHTML='';
  }
  function updateStatus(term=''){
    status.innerHTML=term?`Searching the verified Clyde index from <strong>Newgrounds</strong>.`:`Source: <strong>Newgrounds</strong>. Mature-labeled artwork is not previewed here and remains on the original source page.`;
  }
